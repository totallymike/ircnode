...
irc.version = '';
function getVersionFromFile (err, data) {
  if (err !== null) console.log(err);
  else {
    try {
      irc.version = JSON.parse(data).version;
    } catch (e) {
      console.log(e);
    }
  }
  require('child_process').exec("git log -n1 --format=%H", { 'cwd': __dirname + '/../' }, function (err, stdout, stderr) {
    irc.version = (irc.version + ' ' + stdout.split('\n')[0]).trim();
  });
}
fs.exists(__dirname + '/../package.json', function (exists) {
  if (exists)
    fs.readFile(__dirname + '/../package.json', 'utf8', getVersionFromFile);
  else
    fs.exists(__dirname + '/../ircnode/package.json', function (exists) {
      if (exists)
        fs.readFile(__dirname + '/../ircnode/package.json', 'utf8', getVersionFromFile);
      else
        require('child_process').exec("git log -n1 --format=%H", { 'cwd': __dirname + '/../' }, function (err, stdout, stderr) {
          irc.version = (irc.version + ' ' + stdout.split('\n')[0]).trim();
        });
    ...
    var listener = function (act) {
      ...
      for (var p in act.params) {
        if (act.params[p] === nick) {
          var authorized = act.params.pop() === '3';
          if (authorized || gotOneResponse) {
