...
global.irc = irc;
irc.plugins = [];
var filterPlugin = function (name) {
  irc.plugins = irc.plugins.filter(function (element, index, array) {
    return element.name !== name;
  });
};
var loadPlugins = function () {
  var files = fs.readdir(plugin_dir, function (err, files) {
    for (var i = 0, len = files.length; i < len; i += 1) {
      ...
      if (ppath.indexOf('.js', ppath.length - 3) === -1) {
        ...
        if (pluginNum !== -1) {
          if (irc.plugins[pluginNum].handler.toString() === plugin.handler.toString())
            continue;
          else {
            if (irc.debug)
              console.log('Updating plugin: ' + plugin.name);
            irc.emitter.removeAllListeners(plugin.name);
            filterPlugin(plugin.name);
          }
        }
        if (irc.debug)
